...
const GenerateQuestionsAsync = errorHandler(
    async function QuestionService_GenerateQuestionsAsync(authorization, id, version) {
        ...
        if (rearrangedContent.isSuccess === false || typeof rearrangedContent == 'string')
        ...
        const questions = await Promise.all(
            chunks.map((chunk) => openAIService.ChatGenerateQuestionsAsync(chunk)),
        );
        ...
        for (let i = 1; i < flattenQuestions.length; i++) {
            ...
            const result = await CreateAsync(authorization, id, {
...
const EvaluateAnswerAsync = errorHandler(
    async function QuestionService_EvaluateAnswerAsync(authorization, data) {
        const rearrangedContent = await rearrangedContentService.GetByIdAsync(
            authorization,
            data.rearrangedContentId,
        );
...
const CreateAsync = errorHandler(
    async function QuestionService_CreateAsync(authorization, id, data) {
        const rearrangedContent = await rearrangedContentService.GetByIdAsync(
            authorization,
            id,
        );
        ...
        if (questions.length > 0)
            questionNumber = questions[questions.length - 1].QuestionNumber + 1;
...
const GetAllByDocumentAsync = errorHandler(
    async function QuestionService_GetAllByDocumentAsync(authorization, id) {
        const rearrangedContent = await rearrangedContentService.GetByIdAsync(
            authorization,
            id,
        );
...
const GetByIdAsync = errorHandler(
    async function QuestionService_GetByIdAsync(authorization, documentId, questionId) {
        const id = '';
        const rearrangedContent = await rearrangedContentService.GetByIdAsync(
            authorization,
            id,
        ...
        if (!(rearrangedContent.Id == question.RearrangedContentId))
