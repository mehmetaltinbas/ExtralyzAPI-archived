...
const CreateAsync = errorHandler(
    async function RearrangedContentService_CreateAsync(authorization, data) {
        ...
        const rearrangedContents = await GetAllAsync(
            authorization,
            data.documentId,
        );
        console.log(
        ...
        if (rearrangedContents.length > 0)
            version =
                rearrangedContents[rearrangedContents.length - 1].Version + 1;
...
const GetAllAsync = errorHandler(
    async function RearrangedContentService_GetAllAsync(authorization, id) {
        try {
            ...
            console.error(
                "Error in RearrangedDocumentService/GetAllAsync --> ",
                error.stack,
            );
            return {
                success: false,
...
const GetByVersionAsync = errorHandler(
    async function RearrangedContentService_GetByVersionAsync(
        authorization,
        id,
        version,
    ) {
...
const GetByIdAsync = errorHandler(
    async function RearrangedContentService_GetByIdAsync(authorization, id) {
        ...
        if (document.success === false || typeof document == "string")
            return document;
...
const UpdateAsync = errorHandler(
    async function RearrangedContentService_UpdateAsync() {},
);

const DeleteAsync = errorHandler(
    async function RearrangedContentService_DeleteAsync(id) {
