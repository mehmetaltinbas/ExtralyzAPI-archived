...
const ChatGetEmbeddingsAsync = errorHandler(async function OpenAIService_ChatGetEmbeddingsAsync(sentences) {
...
const ChatSummarizeAsync = errorHandler(async function OpenAIService_ChatSummarizeAsync(chunk) {
    ...
    const bestCaseScenario = scenarios.find((scenario) => scenario.difference === bestDifference);
    console.log(`\nChunk ${chunk.number} - The best case scenario --> ${JSON.stringify(bestCaseScenario, null, 2)} \n`);
    summary = bestCaseScenario.summary;
    return summary;
});

const ChatGenerateQuestionsAsync = errorHandler(async function OpenAIService_ChatGenerateQuestion(chunk) {
...
const ChatEvaluateAnswerAsync = errorHandler(async function OpenAIService_ChatEvaluateAnswerAsync(data) {
