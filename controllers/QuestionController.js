...
router.post(
    '/evaluateanswer/:rearrangedcontentid/:questionnumber',
    async function EvaluateAnswer(req, res) {
        const result = await questionService.EvaluateAnswerAsync(req.headers.authorization, {
...
router.get('/getallbydocument/:id', async function GetAllQuestions(req, res) {
    const questions = await questionService.GetAllByDocumentAsync(
        req.headers.authorization,
        req.params.id,
    );
    res.json(questions);
